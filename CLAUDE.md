...
**Nomes de macro sem prefixo, de propósito.** `\figura`, `\natureza`, `\modalidade`, `\removido`, `\incluido`, `\pnum`, `\reivindicacao` e companhia ocupam o espaço global de nomes sem prefixo nenhum, o que num pacote de uso geral seria antipadrão. Aqui é escolha: são a API que quem redige o pedido digita, e o vocabulário da norma em português é justamente o que torna o arquivo de conteúdo legível como a peça que ele é. A proteção contra colisão é o `\newcommand`, que **falha ruidosamente** se algo já tiver o nome — e é por isso que se acrescenta pacote com `\RequirePackage` no `lib/inpitex.sty`, onde a colisão aparece na hora, e não nas raízes. Não renomeie: cada nome desses está em uso nos arquivos de `pedido/` de todo repositório derivado deste template.

**`\ProvidesPackage{lib/inpitex}` com o caminho no nome.** É o que o `\usepackage{lib/inpitex}` das quatro raízes exige; declarar só `inpitex` faria o LaTeX avisar que o pacote pedido e o fornecido divergem. A consequência é que o pacote não se instala numa árvore TEXMF e não responde a `\@ifpackageloaded{inpitex}` — nenhuma das duas coisas é usada aqui, e o template é feito para ser copiado inteiro, não instalado.
